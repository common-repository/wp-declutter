...
Plugin URI: https://wordpress.org/plugins/wp-declutter/
...
class WP_Declutter {
	...
	function template_filter() {
		// wp_head
		$this->remove_actions( 'wp_head', 'wp_head',
			array(
				'feed_links' => 2,
				'feed_links_extra' => 3,
		...
		// Some of the settings in the wp_head group use different hooks
		$this->remove_actions('wp_print_styles', 'wp_head', array(), array('print_emoji_styles'));
	...
	private function remove_actions($hook, $group, $priorities = array(), $keys=array()) {
		...
		$filtered_keys = array_keys($this->options[$group]);
		if($keys){
			$keys = array_intersect($keys, $filtered_keys);
		}
		else {
			$keys = $filtered_keys;
		}
